...
export default function AddProductsPage() {
  ...
  // Volver al paso de productos si se eliminan todos en el paso de precios
  useEffect(() => {
    if (step === "prices" && selectedProductIds.size === 0) {
      setStep("products");
    }
  }, [step, selectedProductIds.size]);
  ...
  const handleAddProducts = async () => {
    ...
    if (invalidProducts.length > 0) {
      alert("Pon un precio a todos los productos");
      return;
    }
  ...
  return (
    <div className="h-[calc(100vh-120px)]">
      {/* Botón de volver */}
      <div className="bg-white p-3 py-3 mb-3 sticky top-0 z-10">
        <button
          onClick={() => {
            if (step === "prices") {
              setStep("products");
          ...
          className="inline-flex items-center gap-2 text-gray-700 hover:text-gray-900 transition-colors bg-blue-50 px-2 sm:px-4 py-1 sm:py-2 rounded-lg"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
          {step === "prices" ? "Volver" : "Volver a productos"}
        </button>
      </div>

      <div className="flex gap-0 h-[calc(100%-40px)] min-h-[500px]">
        {/* Sidebar de Categorías - Visible solo en paso de productos */}
        {step === "products" && (
          <div className="w-[90px] sm:w-[110px] flex-shrink-0 fixed left-0 h-[calc(100vh-120px)]">
            <div className="bg-white h-full flex flex-col border-r border-gray-200">
              ...
              <div className="flex-1 overflow-y-auto space-y-2 p-2 pb-20">
                {categories.map((category) => (
                  <button
                    ...
                    className={`w-full p-2 text-xs transition-colors ${
                  ...
                  >
                    <div className="flex flex-col items-center gap-1.5">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 relative flex-shrink-0 rounded-full overflow-hidden bg-gray-100 border border-gray-200">
                        <Image
                      ...
                      <div className="w-full text-center">
                        <div className="font-medium text-[12px] sm:text-xs leading-tight line-clamp-2">
                          {category.name}
                        </div>
                      </div>
                    </div>
        ...
        {/* Contenido Principal */}
        <div className={`flex-1 min-w-0 flex flex-col ${step === "products" ? "ml-[90px] sm:ml-[110px] pl-2" : "w-full"}`}>
          {/* Header - Solo visible en pasos 2 y 3 */}
          {step !== "categories" && (
            <div className="mb-3 sm:mb-4">
              <div className="flex flex-col sm:flex-row justify-between items-start gap-2 p-2">
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-gray-900">
                    
                    {step === "products" && "Agrega productos a tu tienda"}
                ...
                {selectedProductIds.size > 0 && (
                  <div className="bg-blue-50 px-2 sm:px-4 py-1 sm:py-2 rounded-lg flex-shrink-0">
                    ...
                    {products.map((product) => {
                      ...
                      return (
                        <div
                          ...
                          className={`relative bg-white border rounded-lg p-1 sm:p-2 cursor-pointer transition-all ${
                            isSelected
                        ...
                        >
                          {isSelected && (
                            <div className="absolute top-1 right-1 sm:top-2 sm:right-2 z-10 bg-blue-600 text-white rounded-full p-1">
                              <svg
          ...
          {step === "prices" && (
            <div className="bg-white rounded-lg shadow flex-1 flex flex-col min-h-0">
              <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-3 sm:space-y-4 pb-20">
                {Array.from(selectedProducts.values()).map((product) => (
                  ...
                  >
                    {/* Header del producto */}
                    <div className="bg-gray-50 p-2 sm:p-3 flex items-center gap-2 sm:gap-3 border-b border-gray-200">
                      <div className="w-10 h-10 sm:w-12 sm:h-12 relative flex-shrink-0 bg-gray-100 rounded overflow-hidden">
                        <ProductImage src={product.imageUrl} alt={product.name} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm sm:text-base font-medium text-gray-900 truncate">{product.name}</p>
                        <p className="text-xs sm:text-sm text-gray-500">
                          {product.productMeasurements.length} unidad(es)
                        </p>
                      </div>
                      <button
                        onClick={() => {
                    ...
                    <div className="p-2 sm:p-3 space-y-2">
                      {product.productMeasurements.length === 0 ? (
                        <div className="text-xs sm:text-sm text-gray-500 py-2 text-center">
                        ...
                        product.productMeasurements.map((pm) => (
                          <div
                            key={pm.id}
                            className="flex items-center justify-between gap-3 p-2 bg-white border border-gray-200 rounded"
                          >
                            <div className="flex-1 min-w-0">
                            ...
                            <div className="w-24 sm:w-28 flex-shrink-0">
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  placeholder="0.00"
                                  className="w-full pl-6 sm:pl-7 pr-2 sm:pr-3 py-1.5 sm:py-2 text-xs sm:text-sm border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                />
                              </div>
                            </div>
                          </div>
      ...
      {/* Botón fijo para el paso de productos */}
      {step === "products" && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 sm:p-4 shadow-lg z-20">
          <button
            ...
            className="w-full px-4 py-3 text-base font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Asignar precios →
          </button>
        </div>
      ...
      {/* Botón Listo fijo para el paso de precios */}
      {step === "prices" && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 sm:p-4 shadow-lg z-20">
          <button
            onClick={handleAddProducts}
            disabled={adding}
            className="w-full px-4 py-3 text-base font-medium bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {adding ? "Agregando productos..." : "Listo, agregar productos"}
          </button>
        </div>
      )}
    </div>
