...
export default function NewProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Agregar las unidades de medida si hay alguna configurada
      if (measurements.length > 0) {
        for (const measurement of measurements) {
          await fetch(`/api/admin/products/${createdProduct.id}/measurements`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              measurementUnitId: measurement.measurementUnitId,
              isDefault: measurement.isDefault,
              minQuantity: measurement.minQuantity || null,
              stepQuantity: measurement.stepQuantity || null,
            }),
          });
        }
      }
